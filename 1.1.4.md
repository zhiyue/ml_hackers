...
令人惊奇的是，数据时间跨度相当的长，最早的UFO目击时间在1400年！基于这个情况，下一个问题是：这些数据在不同时间是如何分布的？我们是否有必要对所有时间段的数据进行分析？最快捷的方式就是画一个柱状图（histogram）。我们会在下一张对柱状图进行更细致的描述，现在我们只需要知道柱状图可以按照不同的考察对象根据指定的预定义间隔画出的频率。我们这里使用时间作为考察对象：
